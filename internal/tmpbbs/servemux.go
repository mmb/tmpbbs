...
func NewServeMux(vipr *viper.Viper, staticFS embed.FS, postStore *PostStore,
	...
	rootPost := postStore.rootPost()
	serveMux.Handle("GET /static/", newImmutableGetHandler(http.StripPrefix("/static", http.FileServerFS(staticDir)),
		rootPost.time))
	serveMux.Handle("GET /robots.txt", newImmutableGetHandler(http.FileServerFS(staticDir), rootPost.time))
	...
	if vipr.GetBool("emoji") {
		serveMux.Handle("GET /emoji-suggest", newImmutableGetHandler(newEmojiSuggestHandler(), rootPost.time))
	}

	if vipr.GetBool("qr-codes") {
		serveMux.Handle("GET /qr", newImmutableGetHandler(newQRCodeGetHandler(), rootPost.time))
	}

	if vipr.GetBool("serve-binary") {
		serveMux.Handle("GET /self", newImmutableGetHandler(newSelfGetHandler(), rootPost.time))
	}
